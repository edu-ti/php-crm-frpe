 ...
 function handle_create_agendamento($pdo, $data) {
     ...
     if ($success && isset($lastId)) {
         ...
         if ($agendamento) {
             ...
             $targetRoles = [];
             $recipientEmails = []; // Inicializa

             try {
                 // Define roles alvo baseados no tipo de agendamento
                 if (strtolower($tipoAgendamento) === 'treinamento') {
                     // Notifica apenas Analista, Gestor, Comercial, Especialista para treinamentos
                     $targetRoles = ['Analista', 'Gestor', 'Comercial', 'Especialista'];
                 } else {
                     // Notifica todos, exceto Marketing, para outros tipos
                 ...
                 error_log("DEBUG: [Agenda Create] Roles alvo para notificação: " . implode(', ', $targetRoles));

                 // Busca emails dos roles alvo
                 if (!empty($targetRoles)) {
                     ...
                     if (!empty($recipientEmails)) {
                         ...
                         catch (Exception $e) { $dataHoraFormatada = 'Data/Hora Inválida'; }

                         $subject = "[CRM FR] Novo Agendamento: " . $agendamento['titulo'];
                         $htmlBody = "<h2>Novo Agendamento Criado no CRM</h2>" .
                                     "<p>Um novo agendamento foi adicionado ao sistema:</p>" .
                                     "<ul>" .
                                         ...
                                         "<li><strong>Direcionado para:</strong> " . htmlspecialchars($agendamento['para_usuario_nomes'] ?: 'N/A') . "</li>" . // Usa nomes concatenados
                         ...
                         if (function_exists('send_email_notification')) {
                              if (!send_email_notification($recipientEmails, $subject, $htmlBody)) {
                                   error_log("AVISO: [Agenda Create] Falha ao enviar notificação via send_email_notification para ID: " . $lastId);
                                   // Poderia ter um fallback para a fila aqui se necessário
                              } else {
                                   error_log("DEBUG: [Agenda Create] Notificação enviada via send_email_notification para ID: " . $lastId);
                              }
                         } else {
                              error_log("ERRO: [Agenda Create] Função send_email_notification não encontrada. Não foi possível enviar e-mail.");
                         }

                     } else { error_log("AVISO: [Agenda Create] Nenhum email encontrado para roles alvo. Notificação não enviada. ID: " . $lastId); }
                 } else { error_log("AVISO: [Agenda Create] Nenhum role alvo definido para notificação. ID: " . $lastId); }

             } catch (PDOException $e) {
                 error_log("ERRO DB: [Agenda Create] Falha ao buscar roles/emails para notificação: " . $e->getMessage());
             } catch (Throwable $t) { // Captura erros gerais também
                 error_log("ERRO PHP: [Agenda Create] Falha na lógica de notificação: " . $t->getMessage() . " em " . $t->getFile() . ":" . $t->getLine());
             }
         } else {
             error_log("ERRO: [Agenda Create] Não foi possível buscar detalhes do agendamento após inserção. Notificação não enviada.");
         }
 ...
 function handle_update_agendamento($pdo, $data) {
     ...
     if ($data === null) { json_response(['success' => false, 'error' => 'Nenhum dado recebido.'], 400); return; }
     $required_fields = ['id', 'titulo', 'data_agendamento', 'hora_agendamento', 'tipo']; // id é necessário
     foreach ($required_fields as $field) { if (empty($data[$field])) { json_response(['success' => false, 'error' => "Campo obrigatório ausente: {$field}"], 400); return; } }
     // ***** ALTERAÇÃO: Valida array de usuários *****
     $para_usuario_ids = $data['para_usuario_ids'] ?? [];
     $para_usuario_ids = array_filter($para_usuario_ids, function($id){ return is_numeric($id) && $id > 0; });
     if (empty($para_usuario_ids)) {
         json_response(['success' => false, 'error' => "Selecione pelo menos um usuário válido para direcionar."], 400);
         return;
     }
     ...
     if ($success) {
         // ***** ALTERAÇÃO: Busca detalhes incluindo nomes e IDs concatenados *****
          ...
          try {
             ...
             $agendamento = $stmt_updated->fetch(PDO::FETCH_ASSOC);
              ...
              if ($agendamento && !empty($agendamento['usuarios_associados_ids'])) {
                 $agendamento['usuarios_associados'] = explode(',', $agendamento['usuarios_associados_ids']);
             } else if ($agendamento) { $agendamento['usuarios_associados'] = []; }

          } catch (PDOException $e) { error_log("ERRO DB: [Agenda Update] Falha ao buscar detalhes atualizados: " . $e->getMessage()); }
          // ***** FIM DA ALTERAÇÃO *****
         ...
         if ($agendamento) {
              // ... (Lógica idêntica a 'create' para determinar $targetRoles e buscar $recipientEmails) ...
              $tipoAgendamento = $agendamento['tipo'];
              $targetRoles = [];
              if (strtolower($tipoAgendamento) === 'treinamento') {
                   $targetRoles = ['Analista', 'Gestor', 'Comercial', 'Especialista'];
              ...
              if (!empty($targetRoles)) {
                  ...
                  if (!empty($recipientEmails)) {
                      ...
                      $htmlBody = "<h2>Agendamento Atualizado no CRM</h2>" .
                                 "<p>O seguinte agendamento foi modificado por " . htmlspecialchars($_SESSION['nome']) . ":</p>" . // Adiciona quem modificou
                                 "<ul>" .
                                     "<li><strong>Título:</strong> " . htmlspecialchars($agendamento['titulo']) . "</li>" .
                                     "<li><strong>Tipo:</strong> " . htmlspecialchars($agendamento['tipo']) . "</li>" .
                                     "<li><strong>Data e Hora:</strong> " . $dataHoraFormatada . "</li>" .
                                     "<li><strong>Direcionado para:</strong> " . htmlspecialchars($agendamento['para_usuario_nomes'] ?: 'N/A') . "</li>" . // Usa nomes concatenados
                                     "<li><strong>Criado originalmente por:</strong> " . htmlspecialchars($agendamento['criado_por_nome'] ?: 'N/A') . "</li>" . // Indica quem criou
                                     (!empty($agendamento['oportunidade_titulo']) ? "<li><strong>Oportunidade Associada:</strong> " . htmlspecialchars($agendamento['oportunidade_titulo']) . "</li>" : "") .
                                     "<li><strong>Observações:</strong> " . nl2br(htmlspecialchars($agendamento['descricao'] ?: 'Nenhuma')) . "</li>" .
                                 "</ul>" .
                                 "<p>Pode ver mais detalhes na Agenda do CRM.</p>";
                      ...
                      if (function_exists('send_email_notification')) {
                           if (!send_email_notification($recipientEmails, $subject, $htmlBody)) {
                                error_log("AVISO: [Agenda Update] Falha ao enviar notificação via send_email_notification para ID: " . $agendamentoId);
                           } else {
                                error_log("DEBUG: [Agenda Update] Notificação enviada via send_email_notification para ID: " . $agendamentoId);
                           }
                      } else {
                           error_log("ERRO: [Agenda Update] Função send_email_notification não encontrada.");
                      }

                  } else { error_log("AVISO: [Agenda Update] Nenhum email encontrado para roles alvo. Notificação não enviada. ID: " . $agendamentoId); }
             } else { error_log("AVISO: [Agenda Update] Nenhum role alvo definido para notificação. ID: " . $agendamentoId); }
         } else { error_log("ERRO: [Agenda Update] Não foi possível buscar detalhes atualizados do agendamento. Notificação não enviada."); }
         // --- FIM DA LÓGICA DE ENVIO DE EMAIL ---
 ...
 function handle_delete_agendamento($pdo, $data) {
     ...
     if (empty($data['id'])) { json_response(['success' => false, 'error' => 'ID do agendamento não fornecido.'], 400); return; }
     $agendamentoId = $data['id'];
     ...
     if ($success && $agendamento_deleted) {
         ...
         if (!empty($targetRoles)) {
             ...
             if (!empty($recipientEmails)) {
                 $dataHoraFormatada = '';
                 try { $dt = new DateTime($agendamento_deleted['data_inicio']); $dataHoraFormatada = $dt->format('d/m/Y \à\s H:i'); }
                 catch (Exception $e) { $dataHoraFormatada = 'Data/Hora Inválida'; }
             ...
             } else { error_log("AVISO: [Agenda Delete] Nenhum email encontrado para roles alvo. Notificação não enviada. ID: " . $agendamentoId); }
        } else { error_log("AVISO: [Agenda Delete] Nenhum role alvo definido para notificação. ID: " . $agendamentoId); }
